...
static const char* g_TAG = "PCAL9555_Test";
#define TAG g_TAG  // TestFramework.h macros expect TAG
...
static std::unique_ptr<PCAL95555Driver> create_test_driver() noexcept {
  if (!g_i2c_bus) {
    ESP_LOGE(g_TAG, "I2C bus not initialized");
    return nullptr;
  }
  ...
  if (!driver) {
    ESP_LOGE(g_TAG, "Failed to create driver instance");
    return nullptr;
  }
  ...
  if (errors != 0) {
    ESP_LOGW(g_TAG, "Driver has error flags: 0x%04X (device may not be accessible at this address)",
             errors);
...
[[maybe_unused]] static bool verify_pin_state(PCAL95555Driver& driver, uint8_t pin, bool expected,
  ...
  if (actual != expected) {
    ESP_LOGE(g_TAG, "%s: Pin %d state mismatch - expected %s, got %s", context, pin,
...
static bool test_i2c_bus_initialization() noexcept {
  ...
  ESP_LOGI(g_TAG, "[OK] I2C bus initialized successfully");
  ESP_LOGI(g_TAG, "   SDA:GPIO%d, SCL:GPIO%d", config.sda_pin, config.scl_pin);
  ESP_LOGI(g_TAG, "   Address pins: A0=GPIO%d, A1=GPIO%d, A2=GPIO%d", config.a0_pin, config.a1_pin,
           config.a2_pin);
  return true;
}
 ...
 *
 * Initialization is lazy: the first call to a method that needs I2C (e.g. ResetToDefault)
 * triggers EnsureInitialized() -> initialize(). create_test_driver() already calls
 * ResetToDefault() once; we call it again here to exercise reset-after-init. A short
 * delay after SetAddressPins (in the bus layer) avoids NACK on the first I2C read.
 * NACK on register 0x4F during variant detection is expected when using PCA9555.
 */
static bool test_driver_initialization() noexcept {
  ESP_LOGI(g_TAG, "Testing driver initialization...");
  ...
  // Reset to default (driver already initialized by create_test_driver()->ResetToDefault)
  ...
  if (errors != 0) {
    ESP_LOGW(g_TAG, "Driver has error flags: 0x%04X", errors);
  ...
  ESP_LOGI(g_TAG, "[OK] Driver initialized successfully");
  ESP_LOGI(g_TAG, "   Detected chip variant: %s", variant_name);
  ESP_LOGI(g_TAG, "   Agile I/O support: %s", g_driver->HasAgileIO() ? "YES" : "NO");
  return true;
}
...
static bool test_single_pin_direction() noexcept {
  ESP_LOGI(g_TAG, "Testing single pin direction configuration...");

  if (!g_driver) {
    ESP_LOGE(g_TAG, "Driver not initialized");
    return false;
  }
  ...
  ESP_LOGI(g_TAG, "[OK] Single pin direction tests passed");
  return true;
}
...
static bool test_multiple_pin_direction() noexcept {
  ESP_LOGI(g_TAG, "Testing multiple pin direction configuration...");
  ...
  if (!g_driver->SetMultipleDirections(mask, GPIODir::Output)) {
    ESP_LOGE(g_TAG, "Failed to set multiple pins to output");
    return false;
  }
  ...
  if (!g_driver->SetMultipleDirections(mask, GPIODir::Input)) {
    ESP_LOGE(g_TAG, "Failed to set multiple pins to input");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Multiple pin direction tests passed");
  return true;
}
...
static bool test_pin_write() noexcept {
  ESP_LOGI(g_TAG, "Testing pin write operations...");
  ...
  if (!g_driver->WritePin(test_pin, true)) {
    ESP_LOGE(g_TAG, "Failed to write HIGH to pin %d", test_pin);
    return false;
  }
  vTaskDelay(pdMS_TO_TICKS(10));
  ...
  if (!g_driver->WritePin(test_pin, false)) {
    ESP_LOGE(g_TAG, "Failed to write LOW to pin %d", test_pin);
    return false;
  }
  vTaskDelay(pdMS_TO_TICKS(10));

  ESP_LOGI(g_TAG, "[OK] Pin write tests passed");
  return true;
}
...
static bool test_pin_read() noexcept {
  ESP_LOGI(g_TAG, "Testing pin read operations...");
  ...
  ESP_LOGI(g_TAG, "[OK] Pin read tests passed");
  return true;
}
...
static bool test_pin_toggle() noexcept {
  ESP_LOGI(g_TAG, "Testing pin toggle operations...");
  ...
  for (int i = 0; i < 5; ++i) {
    if (!g_driver->TogglePin(test_pin)) {
      ESP_LOGE(g_TAG, "Failed to toggle pin %d", test_pin);
      return false;
    }
    vTaskDelay(pdMS_TO_TICKS(100));
  }

  ESP_LOGI(g_TAG, "[OK] Pin toggle tests passed");
  return true;
}
...
static bool test_pull_resistor_config() noexcept {
  ESP_LOGI(g_TAG, "Testing pull resistor configuration...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Pull resistor config requires PCAL9555A (detected PCA9555)");
  ...
  if (!g_driver->SetPullEnable(test_pin, true)) {
    ESP_LOGE(g_TAG, "Failed to enable pull on pin %d", test_pin);
    return false;
  }

  if (!g_driver->SetPullDirection(test_pin, true)) {
    ESP_LOGE(g_TAG, "Failed to set pull-up on pin %d", test_pin);
    return false;
  }

  // Test enabling pull-down
  if (!g_driver->SetPullDirection(test_pin, false)) {
    ESP_LOGE(g_TAG, "Failed to set pull-down on pin %d", test_pin);
    return false;
  }

  // Test disabling pull
  if (!g_driver->SetPullEnable(test_pin, false)) {
    ESP_LOGE(g_TAG, "Failed to disable pull on pin %d", test_pin);
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Pull resistor tests passed");
  return true;
}
...
static bool test_drive_strength() noexcept {
  ESP_LOGI(g_TAG, "Testing drive strength configuration...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Drive strength config requires PCAL9555A (detected PCA9555)");
    return true;  // Pass - feature not available on this chip
  }
  ...
  for (uint8_t level = 0; level < 4; ++level) {
    ...
    if (!g_driver->SetDriveStrength(test_pin, ds)) {
      ESP_LOGE(g_TAG, "Failed to set drive strength level %d on pin %d", level, test_pin);
      return false;
    }
    vTaskDelay(pdMS_TO_TICKS(10));
  }

  ESP_LOGI(g_TAG, "[OK] Drive strength tests passed");
  return true;
}
...
static bool test_output_mode() noexcept {
  ESP_LOGI(g_TAG, "Testing output mode configuration...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Output mode config requires PCAL9555A (detected PCA9555)");
    return true;  // Pass - feature not available on this chip
  }

  // Test push-pull mode (default)
  if (!g_driver->SetOutputMode(false, false)) {
    ESP_LOGE(g_TAG, "Failed to set push-pull mode");
    return false;
  }

  // Test open-drain mode for port 0
  if (!g_driver->SetOutputMode(true, false)) {
    ESP_LOGE(g_TAG, "Failed to set open-drain mode for port 0");
    return false;
  }

  // Test open-drain mode for port 1
  if (!g_driver->SetOutputMode(false, true)) {
    ESP_LOGE(g_TAG, "Failed to set open-drain mode for port 1");
    return false;
  }

  // Test open-drain mode for both ports
  if (!g_driver->SetOutputMode(true, true)) {
    ESP_LOGE(g_TAG, "Failed to set open-drain mode for both ports");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Output mode tests passed");
  return true;
}
...
static bool test_polarity_inversion() noexcept {
  ESP_LOGI(g_TAG, "Testing input polarity inversion...");
  ...
  if (!g_driver->SetPinPolarity(test_pin, Polarity::Normal)) {
    ESP_LOGE(g_TAG, "Failed to set normal polarity on pin %d", test_pin);
    return false;
  }

  // Test inverted polarity
  if (!g_driver->SetPinPolarity(test_pin, Polarity::Inverted)) {
    ESP_LOGE(g_TAG, "Failed to set inverted polarity on pin %d", test_pin);
    return false;
  }
  ...
  if (!g_driver->SetMultiplePolarities(mask, Polarity::Inverted)) {
    ESP_LOGE(g_TAG, "Failed to set inverted polarity on multiple pins");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Polarity inversion tests passed");
  return true;
}
...
static bool test_input_latch() noexcept {
  ESP_LOGI(g_TAG, "Testing input latch functionality...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Input latch config requires PCAL9555A (detected PCA9555)");
    return true;  // Pass - feature not available on this chip
  }
  ...
  if (!g_driver->EnableInputLatch(test_pin, true)) {
    ESP_LOGE(g_TAG, "Failed to enable input latch on pin %d", test_pin);
    return false;
  }

  // Test disabling latch
  if (!g_driver->EnableInputLatch(test_pin, false)) {
    ESP_LOGE(g_TAG, "Failed to disable input latch on pin %d", test_pin);
    return false;
  }
  ...
  if (!g_driver->EnableMultipleInputLatches(mask, true)) {
    ESP_LOGE(g_TAG, "Failed to enable input latch on multiple pins");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Input latch tests passed");
  return true;
}
...
static bool test_interrupt_mask_config() noexcept {
  ESP_LOGI(g_TAG, "Testing interrupt mask configuration...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Interrupt mask config requires PCAL9555A (detected PCA9555)");
    return true;
  }
  ...
  if (!g_driver->ConfigureInterruptMask(mask)) {
    ESP_LOGE(g_TAG, "Failed to configure interrupt mask");
    return false;
  }
  ...
  if (!g_driver->ConfigureInterruptMask(mask)) {
    ESP_LOGE(g_TAG, "Failed to enable interrupts on all pins");
    return false;
  }
  ...
  if (!g_driver->ConfigureInterruptMask(mask)) {
    ESP_LOGE(g_TAG, "Failed to disable interrupts on all pins");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Interrupt mask configuration tests passed");
  return true;
}
...
static bool test_interrupt_status() noexcept {
  ESP_LOGI(g_TAG, "Testing interrupt status reading...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Interrupt status reading requires PCAL9555A (detected PCA9555)");
    return true;
  }
  ...
  ESP_LOGI(g_TAG, "Initial interrupt status: 0x%04X", status);

  // Reading interrupt status clears it, so read again
  status = g_driver->GetInterruptStatus();
  ESP_LOGI(g_TAG, "Interrupt status after read: 0x%04X", status);

  ESP_LOGI(g_TAG, "[OK] Interrupt status reading tests passed");
  return true;
}
...
static bool test_pin_interrupt_callbacks() noexcept {
  ESP_LOGI(g_TAG, "Testing per-pin interrupt callback registration...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Pin interrupt callbacks require PCAL9555A (detected PCA9555)");
    return true;
  }
  ...
  if (!callback_registered) {
    ESP_LOGE(g_TAG, "Failed to register pin interrupt callbacks");
    return false;
  }
  ...
  g_driver->SetInterruptCallback([](uint16_t status) {
    ...
    ESP_LOGI(g_TAG, "Global interrupt callback: status=0x%04X", status);
  });

  ESP_LOGI(g_TAG, "[OK] Per-pin interrupt callback registration tests passed");
  ESP_LOGI(g_TAG, "   Registered callbacks:");
  ESP_LOGI(g_TAG, "   - Pin 0: Rising edge");
  ESP_LOGI(g_TAG, "   - Pin 1: Falling edge");
  ESP_LOGI(g_TAG, "   - Pin 2: Both edges");
  ESP_LOGI(g_TAG, "   - Pin 3: Rising edge");
  return true;
}
...
static bool test_interrupt_handler_registration() noexcept {
  ESP_LOGI(g_TAG, "Testing interrupt handler registration...");

  if (!g_driver || !g_i2c_bus) {
    ESP_LOGE(g_TAG, "Driver or bus not initialized");
    return false;
  }
  ...
  if (!g_i2c_bus->SetupInterruptPin(int_pin)) {
    ESP_LOGW(g_TAG, "Failed to setup interrupt pin GPIO %d (may not be connected)", int_pin);
  ...
  if (!g_driver->RegisterInterruptHandler()) {
    ESP_LOGW(g_TAG, "Failed to register interrupt handler");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Interrupt handler registered successfully on GPIO %d", int_pin);
  ESP_LOGI(g_TAG, "   Interrupts will now be processed automatically when INT pin fires");
  return true;
}
...
static bool test_interrupt_callback_unregistration() noexcept {
  ESP_LOGI(g_TAG, "Testing interrupt callback unregistration...");
  ...
  if (!g_driver->UnregisterPinInterrupt(5)) {
    ESP_LOGE(g_TAG, "Failed to unregister callback");
    return false;
  }

  // Try to unregister again (should return false)
  if (g_driver->UnregisterPinInterrupt(5)) {
    ESP_LOGW(g_TAG, "Unexpected success unregistering already-unregistered callback");
  }

  // Test invalid pin
  if (g_driver->UnregisterPinInterrupt(16)) {
    ESP_LOGE(g_TAG, "Unexpected success unregistering invalid pin");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] Interrupt callback unregistration tests passed");
  return true;
}
...
static bool test_interrupt_config() noexcept {
  ESP_LOGI(g_TAG, "Testing interrupt configuration...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Interrupt config requires PCAL9555A (detected PCA9555)");
    return true;
  }
  ...
  uint16_t status = g_driver->GetInterruptStatus();
  ESP_LOGI(g_TAG, "Interrupt status: 0x%04X", status);

  ESP_LOGI(g_TAG, "[OK] Interrupt configuration tests passed");
  return true;
}
...
static bool test_port_operations() noexcept {
  ESP_LOGI(g_TAG, "Testing port-level operations...");
  ...
  ESP_LOGI(g_TAG, "[OK] Port operation tests passed");
  return true;
}
...
static bool test_write_pins_multi() noexcept {
  ESP_LOGI(g_TAG, "Testing WritePins (initializer_list)...");
...
static bool test_read_pins_multi() noexcept {
  ESP_LOGI(g_TAG, "Testing ReadPins (initializer_list)...");
  ...
  if (results.empty()) {
    ESP_LOGE(g_TAG, "ReadPins returned empty");
    return false;
  }

  for (const auto& [pin, value] : results) {
    ESP_LOGI(g_TAG, "ReadPins: pin %d = %s", pin, value ? "HIGH" : "LOW");
  }

  if (results.size() != 4) {
    ESP_LOGE(g_TAG, "ReadPins returned %d results, expected 4", (int)results.size());
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] ReadPins tests passed");
  return true;
}
...
static bool test_set_directions_multi() noexcept {
  ESP_LOGI(g_TAG, "Testing SetDirections (initializer_list)...");
  ...
  if (!g_driver->SetDirections({
    ...
    ESP_LOGE(g_TAG, "SetDirections failed");
    return false;
  }
  ...
  ESP_LOGI(g_TAG, "Input pins after mixed config: pin1=%d, pin3=%d", pin1, pin3);

  ESP_LOGI(g_TAG, "[OK] SetDirections tests passed");
  return true;
}
...
static bool test_set_polarities_multi() noexcept {
  ESP_LOGI(g_TAG, "Testing SetPolarities (initializer_list)...");
  ...
  if (!g_driver->SetPolarities({
    ...
    ESP_LOGE(g_TAG, "SetPolarities failed");
    return false;
  }
  ...
  if (!g_driver->SetPolarities({
    ...
    ESP_LOGE(g_TAG, "Failed to reset polarities");
    return false;
  }

  ESP_LOGI(g_TAG, "[OK] SetPolarities tests passed");
  return true;
}
...
static bool test_address_management() noexcept {
  ESP_LOGI(g_TAG, "Testing address management...");
  ...
  ESP_LOGI(g_TAG, "Original address: 0x%02X (bits=%d)", original_address, original_bits);

  // Test ChangeAddress(bool, bool, bool) - set A0=HIGH, others LOW -> 0x21
  ESP_LOGI(g_TAG, "Changing address to A0=1, A1=0, A2=0 (0x21)...");
  if (g_driver->ChangeAddress(true, false, false)) {
    ESP_LOGI(g_TAG, "ChangeAddress(bool) succeeded, new address: 0x%02X", g_driver->GetAddress());
    ...
    ESP_LOGW(g_TAG, "ChangeAddress(bool) to 0x21 failed (expected if no device at that address)");
  }
  g_driver->ClearErrorFlags();

  // Restore original address
  ESP_LOGI(g_TAG, "Restoring original address (0x%02X)...", original_address);
  if (!g_driver->ChangeAddress(PCAL9555_A0_LEVEL, PCAL9555_A1_LEVEL, PCAL9555_A2_LEVEL)) {
    ESP_LOGE(g_TAG, "Failed to restore original address!");
    return false;
  }
  ESP_LOGI(g_TAG, "Address restored to 0x%02X", g_driver->GetAddress());

  // Test ChangeAddress(uint8_t) - try same address
  ESP_LOGI(g_TAG, "Testing ChangeAddress(uint8_t) with original address...");
  if (!g_driver->ChangeAddress(original_address)) {
    ESP_LOGE(g_TAG, "ChangeAddress(uint8_t) failed for original address");
    return false;
  }

  // Verify address-based constructor (create a temporary driver)
  ESP_LOGI(g_TAG, "Testing address-based constructor (0x%02X)...", original_address);
  {
    PCAL95555Driver temp_driver(g_i2c_bus.get(), original_address);
    if (!temp_driver.EnsureInitialized()) {
      ESP_LOGE(g_TAG, "Address-based constructor driver failed to initialize");
      return false;
    }
    ESP_LOGI(g_TAG, "Address-based constructor: addr=0x%02X, variant=%s",
             temp_driver.GetAddress(),
             temp_driver.HasAgileIO() ? "PCAL9555A" : "PCA9555");
  }

  ESP_LOGI(g_TAG, "[OK] Address management tests passed");
  return true;
}
...
static bool test_config_and_init() noexcept {
  ESP_LOGI(g_TAG, "Testing SetRetries and EnsureInitialized...");
  ...
  ESP_LOGI(g_TAG, "SetRetries(0) - no retries on I2C failure");

  // Verify driver still works with no retries
  bool pin_val = g_driver->ReadPin(0);
  ESP_LOGI(g_TAG, "ReadPin(0) with 0 retries: %s", pin_val ? "HIGH" : "LOW");

  g_driver->SetRetries(3);
  ESP_LOGI(g_TAG, "SetRetries(3) - 3 retries on I2C failure");

  pin_val = g_driver->ReadPin(0);
  ESP_LOGI(g_TAG, "ReadPin(0) with 3 retries: %s", pin_val ? "HIGH" : "LOW");
  ...
  if (!g_driver->EnsureInitialized()) {
    ESP_LOGE(g_TAG, "EnsureInitialized failed on already-initialized driver");
    return false;
  }
  ESP_LOGI(g_TAG, "EnsureInitialized on already-initialized driver: OK");
  ...
  // Test EnsureInitialized on a fresh driver
  {
    ...
    if (!fresh_driver.EnsureInitialized()) {
      ESP_LOGE(g_TAG, "EnsureInitialized failed on fresh driver");
      return false;
    }
    ESP_LOGI(g_TAG, "EnsureInitialized on fresh driver: OK (variant=%s)",
             fresh_driver.HasAgileIO() ? "PCAL9555A" : "PCA9555");
  }

  ESP_LOGI(g_TAG, "[OK] Configuration and initialization tests passed");
  return true;
}
...
static bool test_multi_pin_pcal_apis() noexcept {
  ESP_LOGI(g_TAG, "Testing multi-pin PCAL9555A initializer_list APIs...");
  ...
  if (!g_driver->HasAgileIO()) {
    ESP_LOGW(g_TAG, "[SKIP] Skipping: Multi-pin PCAL APIs require PCAL9555A (detected PCA9555)");
    return true;
  }

  // --- SetPullEnables ---
  ESP_LOGI(g_TAG, "Testing SetPullEnables...");
  if (!g_driver->SetPullEnables({{0, true}, {1, true}, {2, false}, {3, true}})) {
    ESP_LOGE(g_TAG, "SetPullEnables failed");
    return false;
  }
  ESP_LOGI(g_TAG, "  SetPullEnables: OK");

  // --- SetPullDirections ---
  ESP_LOGI(g_TAG, "Testing SetPullDirections...");
  if (!g_driver->SetPullDirections({{0, true}, {1, false}, {2, true}, {3, false}})) {
    ESP_LOGE(g_TAG, "SetPullDirections failed");
    return false;
  }
  ESP_LOGI(g_TAG, "  SetPullDirections: OK");
  ...
  if (!g_driver->SetDriveStrengths({
    ...
    ESP_LOGE(g_TAG, "SetDriveStrengths failed");
    return false;
  }
  ESP_LOGI(g_TAG, "  SetDriveStrengths: OK");

  // --- ConfigureInterrupt (single pin) ---
  ESP_LOGI(g_TAG, "Testing ConfigureInterrupt (single pin)...");
  g_driver->SetPinDirection(5, GPIODir::Input);
  if (!g_driver->ConfigureInterrupt(5, InterruptState::Enabled)) {
    ESP_LOGE(g_TAG, "ConfigureInterrupt(5, Enabled) failed");
    return false;
  }
  if (!g_driver->ConfigureInterrupt(5, InterruptState::Disabled)) {
    ESP_LOGE(g_TAG, "ConfigureInterrupt(5, Disabled) failed");
    return false;
  }
  ESP_LOGI(g_TAG, "  ConfigureInterrupt: OK");
  ...
  if (!g_driver->ConfigureInterrupts({
    ...
    ESP_LOGE(g_TAG, "ConfigureInterrupts failed");
    return false;
  }
  // Disable all again
  if (!g_driver->ConfigureInterrupts({
    ...
    ESP_LOGE(g_TAG, "ConfigureInterrupts (disable) failed");
    return false;
  }
  ESP_LOGI(g_TAG, "  ConfigureInterrupts: OK");

  // --- EnableInputLatches ---
  ESP_LOGI(g_TAG, "Testing EnableInputLatches...");
  if (!g_driver->EnableInputLatches({{4, true}, {5, false}, {6, true}, {7, false}})) {
    ESP_LOGE(g_TAG, "EnableInputLatches failed");
    return false;
  }
  // Disable all latches
  if (!g_driver->EnableInputLatches({{4, false}, {5, false}, {6, false}, {7, false}})) {
    ESP_LOGE(g_TAG, "EnableInputLatches (disable) failed");
    return false;
  }
  ESP_LOGI(g_TAG, "  EnableInputLatches: OK");

  ESP_LOGI(g_TAG, "[OK] Multi-pin PCAL9555A API tests passed");
  return true;
}
...
static bool test_interactive_input() noexcept {
  ESP_LOGI(g_TAG, "");
  ESP_LOGI(g_TAG, "╔══════════════════════════════════════════════════════════════╗");
  ESP_LOGI(g_TAG, "║              INTERACTIVE INPUT TEST                          ║");
  ESP_LOGI(g_TAG, "║                                                              ║");
  ESP_LOGI(g_TAG, "║  This test requires a momentary push-button connected        ║");
  ESP_LOGI(g_TAG, "║  between PCA9555 IO0_0 (pin 0) and GND.                     ║");
  ESP_LOGI(g_TAG, "║                                                              ║");
  ESP_LOGI(g_TAG, "║  If using PCAL9555A, internal pull-up will be enabled.       ║");
  ESP_LOGI(g_TAG, "║  If using PCA9555, an external pull-up resistor is needed.   ║");
  ESP_LOGI(g_TAG, "╚══════════════════════════════════════════════════════════════╝");
  ...
  if (!g_driver->SetPinDirection(BUTTON_PIN, GPIODir::Input)) {
    ESP_LOGE(g_TAG, "Failed to set pin %d as input", BUTTON_PIN);
    return false;
  }
  ...
  if (g_driver->HasAgileIO()) {
    ...
    ESP_LOGI(g_TAG, "Internal pull-up enabled on pin %d (PCAL9555A)", BUTTON_PIN);
  } else {
    ESP_LOGW(g_TAG, "PCA9555 detected: ensure external pull-up on pin %d", BUTTON_PIN);
  }

  // Read initial state
  bool initial = g_driver->ReadPin(BUTTON_PIN);
  ESP_LOGI(g_TAG, "Pin %d initial state: %s (expected HIGH if pull-up active)",
           BUTTON_PIN, initial ? "HIGH" : "LOW");

  // Wait for button press (pin goes LOW when pressed)
  ESP_LOGI(g_TAG, "");
  ESP_LOGI(g_TAG, ">>> Press the button on pin %d within 10 seconds... <<<", BUTTON_PIN);
  ESP_LOGI(g_TAG, "");
  ...
  for (int i = 0; i < 100; ++i) { // 100 * 100ms = 10 seconds
    ...
    if (!state) { // LOW = pressed (active-low with pull-up)
      ESP_LOGI(g_TAG, "Button press detected on pin %d at t=%d ms!", BUTTON_PIN, i * 100);
      button_detected = true;

      // Wait for release
      ESP_LOGI(g_TAG, "Waiting for button release...");
      while (!g_driver->ReadPin(BUTTON_PIN)) {
        vTaskDelay(pdMS_TO_TICKS(50));
      }
      ESP_LOGI(g_TAG, "Button released.");
      break;
    }

    // Print countdown every 2 seconds
    if (i % 20 == 0 && i > 0) {
      ESP_LOGI(g_TAG, "  Waiting... %d seconds remaining", (100 - i) / 10);
    }
    vTaskDelay(pdMS_TO_TICKS(100));
  }

  if (!button_detected) {
    ESP_LOGW(g_TAG, "No button press detected within 10 seconds (this is OK if no button connected)");
    ESP_LOGW(g_TAG, "Skipping interactive verification - ReadPin still exercises the I2C path");
  } else {
    ESP_LOGI(g_TAG, "[OK] Interactive button test verified: ReadPin detects physical state changes");
  }

  // Test HandleInterrupt explicitly (simulate by calling directly)
  ESP_LOGI(g_TAG, "Testing HandleInterrupt (explicit call)...");
  g_driver->HandleInterrupt(); // Should not crash; processes pin state changes
  ESP_LOGI(g_TAG, "  HandleInterrupt: OK (no crash)");

  ESP_LOGI(g_TAG, "[OK] Interactive input tests passed");
  return true;
}
...
static bool test_error_handling() noexcept {
  ESP_LOGI(g_TAG, "Testing error handling...");
  ...
  ESP_LOGI(g_TAG, "  Test: Invalid pin index (pin 16)...");
  bool result = g_driver->SetPinDirection(16, GPIODir::Output);
  if (result) {
    ESP_LOGW(g_TAG, "  Unexpected success with invalid pin");
  }
  uint16_t errors = g_driver->GetErrorFlags();
  ESP_LOGI(g_TAG, "  Error flags after invalid pin: 0x%04X (expect InvalidPin=0x0001)", errors);
  g_driver->ClearErrorFlags();

  // Test 2: Invalid pin for ReadPin and WritePin
  ESP_LOGI(g_TAG, "  Test: Invalid pin read/write (pin 16, 17)...");
  g_driver->ReadPin(16);
  g_driver->WritePin(17, true);
  g_driver->TogglePin(18);
  errors = g_driver->GetErrorFlags();
  ESP_LOGI(g_TAG, "  Error flags after invalid R/W/T: 0x%04X", errors);
  g_driver->ClearErrorFlags();

  // Test 3: ClearErrorFlags with specific mask
  ESP_LOGI(g_TAG, "  Test: ClearErrorFlags with specific mask...");
  g_driver->SetPinDirection(16, GPIODir::Output); // sets InvalidPin
  errors = g_driver->GetErrorFlags();
  ESP_LOGI(g_TAG, "  Before selective clear: 0x%04X", errors);
  g_driver->ClearErrorFlags(0x0001); // Clear only InvalidPin
  errors = g_driver->GetErrorFlags();
  ESP_LOGI(g_TAG, "  After clearing InvalidPin: 0x%04X (expect 0x0000)", errors);
  g_driver->ClearErrorFlags();

  // Test 4: UnsupportedFeature error on PCA9555
  if (!g_driver->HasAgileIO()) {
    ESP_LOGI(g_TAG, "  Test: UnsupportedFeature error (PCA9555)...");
    result = g_driver->SetDriveStrength(0, DriveStrength::Level2);
    if (result) {
      ESP_LOGW(g_TAG, "  Unexpected success for SetDriveStrength on PCA9555");
    }
    errors = g_driver->GetErrorFlags();
    ESP_LOGI(g_TAG, "  Error flags: 0x%04X (expect UnsupportedFeature=0x0010)", errors);
    g_driver->ClearErrorFlags();
  } else {
    ESP_LOGI(g_TAG, "  Skip: UnsupportedFeature test (chip is PCAL9555A)");
  }

  // Test 5: ReadPinStates (private but used by HandleInterrupt, verify no crash)
  ESP_LOGI(g_TAG, "  Test: HandleInterrupt on clean state (no crash expected)...");
  g_driver->HandleInterrupt();
  ESP_LOGI(g_TAG, "  HandleInterrupt completed without crash");

  ESP_LOGI(g_TAG, "[OK] Error handling tests passed");
  return true;
}
...
static bool test_rapid_operations() noexcept {
  ESP_LOGI(g_TAG, "Testing rapid pin operations...");
  ...
  ESP_LOGI(g_TAG, "[OK] Rapid operations tests passed");
  return true;
}
...
extern "C" void app_main(void) {
  ...
  print_test_section_status(g_TAG, "PCAL9555");
  ...
  // Print test summary
  print_test_summary(g_test_results, "PCAL9555", g_TAG);
